...
There are lots of studies on SDN in wired networks, but only 
a few works study on wireless sensor networks due to the following challenges. 
...
which communicate with nodes via one-hop transmission. In this way it improved the 
...
In WSNs, cluster routing method is a kind of protocol, which nodes choose a header periodically, the other node around cluster header (CH) send it's data to cluster header, and the cluster header forwards it to another header, until the root node receives the data. 
