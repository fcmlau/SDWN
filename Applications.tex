...
\subsubsection{Motivation}
Wireless sensor networks (WSN)  generally use muti-hop for data collection,
...
sensor network will soon run out of energy. We call this Energy exausted problem.

A ideal solution for Energy exausted problem is to find a self-adjust routing schedule which 
according to  Energy status of each sensors. Actually, it is a kind of dynamic proggramming 
problem, the optimization target is the loggest lifetime of the whole network, with
complex constrains. We modified LSTM and RNN deep learning model to simulate and simplified
this constrains. Challenges occurs in traditional WSN beacuse of lack
